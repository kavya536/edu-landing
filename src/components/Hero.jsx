...
const Hero = () => (
  <section className="relative overflow-hidden px-6 md:px-12 bg-surface pt-8 pb-16 md:pb-20">
    <div className="max-w-screen-2xl mx-auto grid grid-cols-1 lg:grid-cols-12 gap-12 lg:gap-20 items-center w-full">
      <div className="lg:col-span-7 z-10 text-center lg:text-left pt-0">
        <h1 className="text-4xl sm:text-5xl lg:text-6xl xl:text-7xl font-serif font-bold text-[#353535] tracking-tight mb-4">
        ...
        <div className="flex flex-col sm:flex-row gap-5 px-6 sm:px-0 justify-center lg:justify-start">
          <a href={`${import.meta.env.VITE_STUDENT_HUB_URL}/?view=login`} className="bg-primary text-white px-10 py-5 rounded-md text-base md:text-lg font-bold transition-all flex items-center justify-center shadow-none hover:scale-[1.03]">
            Book a Tutor
          </a>
          <a href={`${import.meta.env.VITE_TUTOR_DASHBOARD_URL}/?view=login`} className="bg-white text-[#353535] px-10 py-5 rounded-md text-base md:text-lg font-bold transition-all flex items-center justify-center border-2 border-primary shadow-none hover:bg-slate-100 hover:scale-[1.03]">
            Join as Tutor
          </a>
        </div>
      </div>
      <div className="lg:col-span-5 relative h-full min-h-[400px] lg:min-h-[600px] mt-10 lg:mt-0">
        ...
        <div className="absolute -bottom-6 md:-bottom-8 -left-4 md:-left-8 bg-surface-container-lowest p-4 md:p-8 rounded-md max-w-[200px] md:max-w-[280px] z-20 border border-slate-100">
